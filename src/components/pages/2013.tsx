import React from "react";

export default function () {
    return <div>
        <h1>Multimatograf 9</h1>
        <img alt="" src="./media/cover2013.jpg"/>
        <h3>
            26-28 апреля 2013, Вологда,
            <br/>выставочный комплекс «Русский Дом»
        </h3>

        <h2>Ссылки</h2>
        <ul>
            <li><a href="http://aka-nyuk.livejournal.com/21316.html">Отчет от организатора (nyuk)</a></li>
            <li><a href="https://goo.gl/photos/o3NSZCVab4pmRTgA8">Фотографии с выставки и конкурсов</a></li>
            <li><a href="https://vk.com/album165018924_173220556">Общий фотоальбом</a>, <a
                href="https://vk.com/album165018924_00">еще фото</a></li>
            <li>Работы конкурса <strong>53c</strong> <a
                href="https://zxart.ee/eng/parties/2013/multimatograf-9">zxart.ee</a>, <a
                href="https://demozoo.org/parties/3478/">demozoo.org</a></li>
            <li><a href="https://www.youtube.com/watch?v=DywU2qNWfqA">Слайдшоу работ [ZX Spectrum + AY]</a> (<a
                href="https://www.pouet.net/prod.php?which=61392">pouet.net</a>)
            </li>
        </ul>

        <hr/>

        <h2>Конкурс &quot;53c&quot;</h2>

        <p>Участникам предлагается за отведенное время создать графическое изображение в атрибутном режиме компьютера ZX
            Spectrum (32х24 точек, 53 цвета). Для создания изображения предоставляется ноутбук со специализированной
            программой.</p>

        <h3>Победители</h3>

        <ol>
            <li>Гомер by Сергей Лета (206pts)</li>
            <li>Злая птица by Вадим кузьминский (205pts)</li>
            <li>Собачка by Наталия Куклина (192pts)</li>
        </ol>

        <div className="row">
            <div className="col-md-4">
                <div className="thumbnail"><a href="http://zxart.ee/eng/authors/s/sergej-leta/gomer/" target="_blank"
                                              title="1. Гомер by Сергей Лета (206pts)"><img
                    alt="1. Гомер by Сергей Лета (206pts)"
                    src="./media/2013/53c/11.png"/> </a>

                    <div className="caption">1. Гомер by Сергей Лета (206pts)</div>
                </div>
            </div>

            <div className="col-md-4">
                <div className="thumbnail"><a href="http://zxart.ee/eng/authors/v/vadim-kuzminskij/zlaja-ptica/"
                                              target="_blank" title="2. Злая птица by Вадим кузьминский (205pts)"><img
                    alt="2. Злая птица by Вадим кузьминский (205pts)"
                    src="./media/2013/53c/10.png"/> </a>

                    <div className="caption">2. Злая птица by Вадим кузьминский (205pts)</div>
                </div>
            </div>

            <div className="col-md-4">
                <div className="thumbnail"><a href="http://zxart.ee/eng/authors/n/natalija-kuklina/sobachka/"
                                              target="_blank" title="3. Собачка by Наталия Куклина (192pts)"><img
                    alt="3. Собачка by Наталия Куклина (192pts)"
                    src="./media/2013/53c/14.png"/> </a>

                    <div className="caption">3. Собачка by Наталия Куклина (192pts)</div>
                </div>
            </div>

            <div className="col-md-4">
                <div className="thumbnail"><a
                    href="http://zxart.ee/eng/authors/s/sasha-marinova/zachem-zhit-esli-garri-potter-neulovim/"
                    target="_blank" title="4. Зачем жить, если Гарри Поттер неуловим by Саша Маринова (187pts)"><img
                    alt="4. Зачем жить, если Гарри Поттер неуловим by Саша Маринова (187pts)"
                    src="./media/2013/53c/13.png"/> </a>

                    <div className="caption">4. Зачем жить, если Гарри Поттер неуловим by Саша
                        Маринова (187pts)
                    </div>
                </div>
            </div>

            <div className="col-md-4">
                <div className="thumbnail"><a href="http://zxart.ee/eng/authors/v/vadim-kuzminskij/tanchiki/"
                                              target="_blank" title="5. Танчики by Вадим кузьминский (176pts)"><img
                    alt="5. Танчики by Вадим кузьминский (176pts)"
                    src="./media/2013/53c/12.png"/> </a>

                    <div className="caption">5. Танчики by Вадим кузьминский (176pts)</div>
                </div>
            </div>

            <div className="col-md-4">
                <div className="thumbnail"><a href="http://zxart.ee/eng/authors/i/irina/chto-eto/" target="_blank"
                                              title="6. Что это... by Ирина (165pts)"><img
                    alt="6. Что это... by Ирина (165pts)"
                    src="./media/2013/53c/04.png"/> </a>

                    <div className="caption">6. Что это... by Ирина (165pts)</div>
                </div>
            </div>

            <div className="col-md-4">
                <div className="thumbnail"><a href="http://zxart.ee/eng/authors/a/anja-s-vanej/belka/" target="_blank"
                                              title="7. Белка! by Аня с Ваней (159pt)"><img
                    alt="7. Белка! by Аня с Ваней (159pt)"
                    src="./media/2013/53c/03.png"/> </a>

                    <div className="caption">7. Белка! by Аня с Ваней (159pt)</div>
                </div>
            </div>

            <div className="col-md-4">
                <div className="thumbnail"><a
                    href="http://zxart.ee/eng/authors/a/aleksandr-egorov/mitki-nikogo-ne-hotjat-pobedit/"
                    target="_blank" title="8. Митьки никого не хотят победить by Александр Егоров (155pts)"><img
                    alt="8. Митьки никого не хотят победить by Александр Егоров (155pts)"
                    src="./media/2013/53c/07.png"/> </a>

                    <div className="caption">8. Митьки никого не хотят победить by Александр
                        Егоров (155pts)
                    </div>
                </div>
            </div>

            <div className="col-md-4">
                <div className="thumbnail"><a href="http://zxart.ee/eng/authors/e/ekaterina-volkova/ne-eshte-griby/"
                                              target="_blank"
                                              title="9. Не ешьте грибы by Екатерина Волкова (131pts)"><img
                    alt="9. Не ешьте грибы by Екатерина Волкова (131pts)"
                    src="./media/2013/53c/05.png"/> </a>

                    <div className="caption">9. Не ешьте грибы by Екатерина Волкова (131pts)</div>
                </div>
            </div>

            <div className="col-md-4">
                <div className="thumbnail"><a href="http://zxart.ee/eng/authors/d/danil-bykov/bez-nazvanija2/"
                                              target="_blank"
                                              title="10. &lt;без названия&gt; by Данил Быков (118pt)"><img
                    alt="10. &lt;без названия&gt; by Данил Быков (118pt)"
                    src="./media/2013/53c/02.png"/> </a>

                    <div className="caption">10. &lt;без названия&gt; by Данил Быков (118pt)</div>
                </div>
            </div>

            <div className="col-md-4">
                <div className="thumbnail"><a href="http://zxart.ee/eng/authors/z/zhenja-eljakov/advenchur-tajm/"
                                              target="_blank" title="11. Адвенчур тайм by Женя Еляков (115pts)"><img
                    alt="11. Адвенчур тайм by Женя Еляков (115pts)"
                    src="./media/2013/53c/01.png"/> </a>

                    <div className="caption">11. Адвенчур тайм by Женя Еляков (115pts)</div>
                </div>
            </div>

            <div className="col-md-4">
                <div className="thumbnail"><a href="http://zxart.ee/eng/authors/f/flast/laka-suna/" target="_blank"
                                              title="11. Лака Суна by Flast (115pts)"><img
                    alt="11. Лака Суна by Flast (115pts)"
                    src="./media/2013/53c/09.png"/> </a>

                    <div className="caption">11. Лака Суна by Flast (115pts)</div>
                </div>
            </div>

            <div className="col-md-4">
                <div className="thumbnail"><a href="http://zxart.ee/eng/authors/u/unknown/dom-na-dereve/"
                                              target="_blank" title="12. Дом на дереве (114pts)"><img
                    alt="12. Дом на дереве (114pts)"
                    src="./media/2013/53c/06.png"/> </a>

                    <div className="caption">12. Дом на дереве (114pts)</div>
                </div>
            </div>

            <div className="col-md-4">
                <div className="thumbnail"><a href="http://zxart.ee/eng/authors/i/ilja-alisov/vzhik/" target="_blank"
                                              title="13. Вжик by Илья Алисов (105pts)"><img
                    alt="13. Вжик by Илья Алисов (105pts)"
                    src="./media/2013/53c/08.png"/> </a>

                    <div className="caption">13. Вжик by Илья Алисов (105pts)</div>
                </div>
            </div>
        </div>

        <hr/>
        <h2>Конкурс фотографий, сделанных посетителями на фестивале</h2>

        <p>В конкурсе участвуют фотографии сделанные посетителями и участниками непосредственно на фестивале. Из
            обработки допустимы только кадрирование, уровни, баланс белого. Спецэффекты не допускаются. Результаты
            конкурса оцениваются зрителями в процессе голосования.</p>

        <h3>Победители</h3>

        <ol>
            <li>Павел и Руслан (201pts)</li>
            <li>Владислав Жданов (184pts)</li>
            <li>Ольга Чунина (135pts)</li>
        </ol>

        <div className="row">
            <div className="col-md-6">
                <div className="thumbnail">
                    <a href="./media/2013/photo-compo/07.jpg"
                       target="_blank" title="1. Павел и Руслан (201pts)"><img
                        alt="1. Павел и Руслан (201pts)"
                        src="./media/2013/photo-compo/07s.jpg"/> </a>
                    <div>1. Павел и Руслан (201pts)</div>
                </div>
            </div>

            <div className="col-md-6">
                <div className="thumbnail">
                    <a href="./media/2013/photo-compo/05.jpg"
                       target="_blank" title="2. Владислав Жданов (184pts)"><img
                        alt="2. Владислав Жданов (184pts)"
                        src="./media/2013/photo-compo/05s.jpg"/> </a>
                    <div>2. Владислав Жданов (184pts)</div>
                </div>
            </div>

            <div className="col-md-6">
                <div className="thumbnail">
                    <a href="./media/2013/photo-compo/01.jpg"
                       target="_blank" title="3. Ольга Чунина (135pts)"><img
                        alt="3. Ольга Чунина (135pts)"
                        src="./media/2013/photo-compo/01s.jpg"/> </a>
                    <div>3. Ольга Чунина (135pts)</div>
                </div>
            </div>

            <div className="col-md-6">
                <div className="thumbnail">
                    <a href="./media/2013/photo-compo/06.jpg"
                       target="_blank" title="4. Руслан Новиков (130pts)"><img
                        alt="4. Руслан Новиков (130pts)"
                        src="./media/2013/photo-compo/06s.jpg"/></a>
                    <div>4. Руслан Новиков (130pts)</div>
                </div>
            </div>

            <div className="col-md-6">
                <div className="thumbnail">
                    <a href="./media/2013/photo-compo/04.jpg"
                       target="_blank" title="5. Роман Чунин (122pts)"><img
                        alt="5. Роман Чунин (122pts)" src="./media/2013/photo-compo/04s.jpg"/>
                    </a>
                    <div>5. Роман Чунин (122pts)</div>
                </div>
            </div>

            <div className="col-md-6">
                <div className="thumbnail">
                    <a href="./media/2013/photo-compo/03.jpg"
                       target="_blank" title="6. Роман Чунин (121pts)"><img
                        alt="6. Роман Чунин (121pts)" src="./media/2013/photo-compo/03s.jpg"/>
                    </a>
                    <div>6. Роман Чунин (121pts)</div>
                </div>
            </div>

            <div className="col-md-6">
                <div className="thumbnail">
                    <a href="./media/2013/photo-compo/02.jpg"
                       target="_blank" title="7. Ольга Чунина (113pts)"><img
                        alt="7. Ольга Чунина (113pts)"
                        src="./media/2013/photo-compo/02s.jpg"/> </a>
                    <div>7. Ольга Чунина (113pts)</div>
                </div>
            </div>
        </div>

        <hr/>
        <h2>game-quest</h2>

        <p>Квест на прохождение игры на компьютере ZX-Spectrum. Участвовать можно было как в одиночку, так и командой.
            Цель каждого тура - пройти игру, или набрать максимальное количество очков.</p>

        <div className="row">
            <div className="col-md-4">
                <div className="thumbnail">
                    <a href="http://www.worldofspectrum.org/infoseekid.cgi?id=0025069" rel="nofollow" target="_self">
                        <img alt="Karlos und Schatze der Azteken"
                             src="./media/2013/game-quest/KarlosUndSchatzeDerAzteken.png"/>
                    </a>
                    <div className="caption">
                        <h3>1-й день</h3>
                        Karlos und Schatze der Azteken
                    </div>
                </div>
            </div>

            <div className="col-md-4">
                <div className="thumbnail">
                    <a href="http://www.worldofspectrum.org/infoseekid.cgi?id=0015215" rel="nofollow" target="_self">
                        <img alt="Fire and Ice" src="./media/2013/game-quest/FireNIce.png"/>
                    </a>
                    <div className="caption">
                        <h3>2-й день</h3>
                        Fire and Ice
                    </div>
                </div>
            </div>

            <div className="col-md-4">
                <div className="thumbnail">
                    <a href="http://www.worldofspectrum.org/infoseekid.cgi?id=0001551" rel="nofollow" target="_self">
                        <img alt="Dynamite Dan" src="./media/2013/game-quest/DynamiteDan.png"/>
                    </a>
                    <div className="caption">
                        <h3>3-й день</h3>
                        Dynamite Dan
                    </div>
                </div>
            </div>
        </div>

        <div className="thumbnail">
            <img alt="Команда «Симсоны»" src="./media/2013/game-quest/simpsons.jpg" title="Команда «Симсоны»"/>
            <div>Команда «Симсоны»</div>
        </div>

        <h3>Победители</h3>
        <ol>
            <li>Симсоны (2522pt)</li>
            <li>Роман Чунин (2030pt)</li>
            <li>Даниил Якушев (866pt)</li>
        </ol>

        <hr/>
        <h2>oldgame compo</h2>

        <p>Традиционная битва на ретрокомпьютерах. Конкурс проводился три раза, каждый день новая игра, новые участники,
            новые победители.</p>

        <div className="row">
            <div className="col-md-6">
                <div className="thumbnail">
                    <img alt="" src="./media/2013/game-compo/1.jpg"/>
                </div>
            </div>

            <div className="col-md-6">
                <div className="thumbnail">
                    <img alt="" src="./media/2013/game-compo/2.jpg"/>
                </div>
            </div>
        </div>

        <h3>Победители</h3>

        <div className="row">
            <div className="col-md-4">
                <div className="thumbnail">
                    <a href="http://www.gamebase64.com/game.php?id=7165" rel="nofollow" target="_self">
                        <img alt="Speed Runner" src="./media/2013/game-compo/Speed_Runner.png"/>
                    </a>
                    <div className="caption">
                        <h3 className="m-0">Day I<br />Speed Runner</h3>
                        <div className="text-muted"><small>Commodore C64</small></div>
                        <ol>
                            <li>Кузьминский Вадим</li>
                            <li>Саша Коробкин</li>
                            <li>Кирилл Андреев</li>
                        </ol>
                    </div>
                </div>
            </div>

            <div className="col-md-4">
                <div className="thumbnail">
                    <a href="http://www.gamebase64.com/game.php?id=1365" rel="nofollow" target="_self">
                        <img alt="Centric" src="./media/2013/game-compo/Centric.png"/>
                    </a>
                    <div className="caption">
                        <h3 className="m-0">Day II<br />Centric</h3>
                        <div className="text-muted"><small>Commodore C64</small></div>
                        <ol>
                            <li>Руслан Шульгин</li>
                            <li>Flast</li>
                            <li>Влад Тисов</li>
                        </ol>
                    </div>
                </div>
            </div>

            <div className="col-md-4">
                <div className="thumbnail">
                    <a href="http://www.youtube.com/watch?v=u9O-3KwSCwc" rel="nofollow" target="_self">
                        <img alt="Gunfight" src="./media/2013/game-compo/Gunfight.jpg"/>
                    </a>
                    <div className="caption">
                        <h3 className="m-0">Day III<br />Gunfight</h3>
                        <div className="text-muted"><small>Atari XL / XE</small></div>
                        <ol>
                            <li>Влад Тисов</li>
                            <li>Даниил Якушев</li>
                            <li>Александр Маринов</li>
                        </ol>
                    </div>
                </div>
            </div>
        </div>

    </div>
        ;
}